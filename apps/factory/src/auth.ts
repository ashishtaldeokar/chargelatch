...
export const FACTORY_ROLE = "factory";

export const oidcConfig: AuthProviderProps = {
  authority: `${keycloakUrl}/realms/${realm}`,
  client_id: import.meta.env.VITE_KEYCLOAK_CLIENT_ID ?? "chargelatch-factory",
  redirect_uri: window.location.origin + "/",
  post_logout_redirect_uri: window.location.origin + "/",
  scope: "openid profile email",
  // PKCE (S256) is oidc-client-ts's default for the code flow.
  automaticSilentRenew: true,
  // Session storage: a shared factory PC should not stay signed in after the tab closes.
