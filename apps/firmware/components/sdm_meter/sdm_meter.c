...
#if CONFIG_SDM_METER_MODEL_SDM630
#define MODEL SDM_MODEL_SDM630
#else
#define MODEL SDM_MODEL_SDM120
#endif

#if CONFIG_SDM_METER_PARITY_EVEN
#define PARITY UART_PARITY_EVEN
#elif CONFIG_SDM_METER_PARITY_ODD
...
esp_err_t sdm_meter_init(void)
{
    ...
    s_block_count = sdm_modbus_plan_blocks(MODEL.registers, MODEL.register_count, s_blocks, MAX_BLOCKS);
    ESP_RETURN_ON_FALSE(s_block_count > 0, ESP_ERR_INVALID_STATE, TAG, "register table for %s is not sorted by address", MODEL.name);

    const uart_config_t config = {
        .baud_rate = CONFIG_SDM_METER_BAUD_RATE,
    ...
    ESP_RETURN_ON_ERROR(uart_param_config(UART_PORT, &config), TAG, "uart_param_config failed");
    ...
    s_ready = true;
    ESP_LOGI(TAG, "%s, slave %d, %d baud on UART%d (tx %d, rx %d, de %d): %u values in %u requests",
             MODEL.name, CONFIG_SDM_METER_SLAVE_ADDRESS, CONFIG_SDM_METER_BAUD_RATE, CONFIG_SDM_METER_UART_NUM,
             CONFIG_SDM_METER_TX_GPIO, CONFIG_SDM_METER_RX_GPIO, CONFIG_SDM_METER_DE_GPIO,
             (unsigned)MODEL.register_count, (unsigned)s_block_count);
    return ESP_OK;
}
...
static sdm_modbus_result_t transact(const sdm_block_t *block, uint8_t *frame)
{
    ...
    sdm_modbus_build_read_input(CONFIG_SDM_METER_SLAVE_ADDRESS, block->start_address, block->register_count, request);
    ...
    const uint32_t transfer_ms = (uint32_t)(expected * 11 * 1000 / CONFIG_SDM_METER_BAUD_RATE);
    const int received = uart_read_bytes(UART_PORT, frame, expected, pdMS_TO_TICKS(transfer_ms + RESPONSE_LATENCY_MS));

    uint8_t exception = 0;
    const sdm_modbus_result_t result = sdm_modbus_check_response(CONFIG_SDM_METER_SLAVE_ADDRESS, block->register_count,
...
esp_err_t sdm_meter_read(sdm_reading_t *reading, const char **last_error)
{
    ...
    for (size_t b = 0; b < s_block_count; b++) {
        ...
        for (size_t i = 0; i < block->value_count; i++) {
            ...
            reading->values[index] = sdm_modbus_float_at(frame, block->start_address, MODEL.registers[index].address);
